...
type TechnologyHardwareDetailHeroProps = {
...
export async function TechnologyHardwareDetailHero({
  productId,
  content,
}: TechnologyHardwareDetailHeroProps) {
  const t = await getTranslations("technology.hardwareDetail");
  const tItem = await getTranslations("technology.hardware.items");
  ...
  return (
    <section className="overflow-hidden bg-gradient-to-b from-brand-50/40 via-surface to-surface">
      <div className="container-wide page-start-padding">
        <Link
          href={{ pathname: "/teknoloji", hash: "donanim" }}
          ...
          <div className="relative lg:sticky lg:top-24">
            <div className="overflow-hidden rounded-2xl border border-border/80 bg-white shadow-lg">
              <div className="relative aspect-[16/10] w-full">
                <Image
                  src={hardwareProductImages[productId]}
                  alt={tItem(`${productId}.imageAlt`)}
                  fill
                  className={hardwareProductImageClass[productId]}
